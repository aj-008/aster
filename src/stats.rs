...
impl fmt::Display for SimStats {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(f, "{:<6} hits={:<10} misses={:<10} hit_rate={:.1}%  MPKI={:.2}",
            "L1D:", self.l1d.hits, self.l1d.misses, self.l1d.hit_rate(), self.l1d.mpki(i))?;
        writeln!(f, "{:<6} hits={:<10} misses={:<10} hit_rate={:.1}%  MPKI={:.2}",
            "L2:", self.l2.hits, self.l2.misses, self.l2.hit_rate(), self.l2.mpki(i))?;
        writeln!(f, "{:<6} hits={:<10} misses={:<10} hit_rate={:.1}%  MPKI={:.2}",
            "LLC:", self.llc.hits, self.llc.misses, self.llc.hit_rate(), self.llc.mpki(i))?;
        Ok(())
    }
}
